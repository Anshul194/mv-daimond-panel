...
import { deleteBlogCategory } from "../../store/slices/blogCategorySlice";
...
const BlogSubCategoryList: React.FC = () => {
  ...
  const {
    ...
    searchQuery,
    filters,
  } = useSelector((state: any) => state.blogSubCategory);
  ...
  const refreshCategoriesList = async () => {
    ...
    try {
      ...
      console.error("Failed to refresh categories list:", error);
    }
  };
  ...
  // Fixed handleDeleteConfirm function
  const handleDeleteConfirm = async () => {
    if (categoryToDelete) {
      ...
      try {
        ...
        await dispatch(
          deleteBlogSubcategory({ id: categoryToDelete?._id })
        ).unwrap();
  ...
  return (
    <div>
      <PageMeta
        ...
        description="List of all  categories in TailAdmin"
      />
            ...
            <tbody className="bg-white divide-y divide-gray-100 dark:bg-gray-900 dark:divide-gray-800">
              {categories.map((cat, idx) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-sm">{cat.BlogCategory.name}</td>
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    <Link to={`/blog/subcategory/edit/${cat?._id}`}>
